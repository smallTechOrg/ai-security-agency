...
class LLMTrace(Base):
    __tablename__='llm_traces'; id:Mapped[int]=mapped_column(Integer, primary_key=True); run_id:Mapped[int]=mapped_column(Integer, default=0, index=True); agent:Mapped[str]=mapped_column(String(60), default=''); provider:Mapped[str]=mapped_column(String(40), default=''); model:Mapped[str]=mapped_column(String(80), default=''); latency_ms:Mapped[int]=mapped_column(Integer, default=0); ok:Mapped[bool]=mapped_column(Boolean, default=False); fallback:Mapped[bool]=mapped_column(Boolean, default=False); operation:Mapped[str]=mapped_column(String(80), default='completion'); detail:Mapped[dict]=mapped_column(JSON, default=dict); created_at:Mapped[datetime]=mapped_column(DateTime, default=datetime.utcnow)
