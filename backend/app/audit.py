...
def run_safe_baseline(db:Session, run_id:int):
    ...
    run.status='running'; run.stage='crawl'; run.progress=10; db.commit(); log(db, run.workspace_id, run.id, 'run.started', {'url':asset.url})
    ...
    db.add(models.Evidence(run_id=run.id, kind='crawl', title='Public crawl inventory', data={'pages':pages,'forms':forms,'elapsed_sec':round(time.time()-started,2)}))
    ...
    run.stage='analysis'; run.progress=65; db.commit()
    ...
    run.app_model=app_model; run.status='completed'; run.stage='report_ready'; run.progress=100; run.cost_estimate_usd=0.04; db.commit(); log(db, run.workspace_id, run.id, 'run.completed', {'findings':db.query(models.Finding).filter_by(run_id=run.id).count(),'cost_estimate_usd':run.cost_estimate_usd})
    return run
